...
def build_net(matrix,sentence,d1,d2,k,dg):
    G=nx.Graph()
    matrix_z=matrix
...
def detect_local_community(graph, seed_node, threshold):
    community = set([seed_node])
    queue = [seed_node]
    while queue:
        node = queue.pop(0)
        neighbors = set(graph.neighbors(node))
        for neighbor in neighbors:
            if neighbor not in community and len(community & set(graph.neighbors(neighbor))) >= threshold:
                community.add(neighbor)
                queue.append(neighbor)
    return community

def filter_edges_above_value(graph, threshold):
    h=graph.copy()
    #filtered_edges = [(u, v, attr) for u, v, attr in graph.edges(data=True) if attr['weight'] >= threshold]
    #filtered_graph = nx.subgraph(graph, filtered_edges)
    edges_to_remove = []
...
def filter_nodes_above_value(graph, threshold):
    h=graph.copy()
...
def scan_graphs(out,g,pre):
    if True:
        ...
        for r in res:
            ...
            asv.append(float(r[2]['weight']))
        asv=np.array(asv)
        ecut=np.percentile(asv, 75)
        ch=0
        for r in res:
            if float(r[0])==0 or float(r[1])==0:continue
        ...
        '''
        # Community detection
        o5=open(out+'/community_'+pre+'_leiden.txt','w+')
        #communities=list(nx.community.label_propagation_communities(g))
        #communities= list(nx.community.louvain_communities(g,seed=123))
        for i in range(3):
            communities = algorithms.leiden(g,weights="weight")
        s=1
        for n in communities.communities:
            for e in n:
                o5.write(str(e)+'\t'+str(s)+'\n')
            s+=1
        plot_graph(communities.communities,g,out+'/community_'+pre+'_leiden.png',labels)
        #exit()
        #print(communities)
        ''' 
        o6=open(out+'/community_'+pre+'_louvain_r2.txt','w+')
        ...
        fg=filter_edges_above_value(g,ecut)

        o9=open(out+'/community_'+pre+'_mannual.txt','w+')
        #coms=algorithms.label_propagation(g)
        coms=detect_local_community(g,top1,2)
        print(coms)
        exit()
        s=1
        coms_n=[]
        ...
        for n in coms:
            ...
            #s+=1
        coms_n.append(tem)
        plot_graph(coms_n,g,out+'/community_'+pre+'_mannual.png',labels)
...
def obtain_important_tokens(matrix,sentence_file,odir,pre):
    '''
    ...
    for e in matrix:
        ...
        for m in e:
            #print(m.shape)
            if dl[c]==1:
                build_net(m,sentence,d1,d2,k,dgp)
                #print(d1)
                #exit()
            else:
                #continue
                build_net(m,sentence,d3,d4,k,dgn)
            tc+=1
        c+=1
